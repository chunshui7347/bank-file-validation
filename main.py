...
def read_xlsx(file_path):
    try:
        ...
        for row1, row2 in zip(sheet1.iter_rows(min_row=2, values_only=True), sheet2.iter_rows(min_row=2, values_only=True)):
            ...
            if bene_acc_no1 and bene_name1 and debit_amount1 and bene_acc_no1 == account_no2 and bene_name1.upper() == str(name_of_payee2).upper() and debit_amount1 == amount2:
                print(f"Match found:")
                print(f"- Bene. Acc. No: {bene_acc_no1}, Bene. Name: {bene_name1}, Debit Amount: {debit_amount1}")
                print(f"- Account No.: {account_no2}, Name of Payee: {name_of_payee2}, Amount: {amount2}")
            else :
